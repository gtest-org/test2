...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def swap_volume(self, old_connection_info,
        ...
        driver_bdm.save(nova_context.get_admin_context())
    ...
    def _live_migration_monitor(self, context, instance, dest, post_method,
        ...
        while True:
            ...
            if info.type == libvirt.VIR_DOMAIN_JOB_NONE:
                ...
                if not finish_event.ready():
                    ...
                    try:
                        ...
                        if isActive:
                            LOG.debug("VM is running, migration failed",
                                      instance=instance)
                        ...
                        if ex.get_error_code() == libvirt.VIR_ERR_NO_DOMAIN:
                            ...
                            LOG.debug("Error %(ex)s, migration failed",
                                      instance=instance)
                            info.type = libvirt.VIR_DOMAIN_JOB_FAILED
            ...
            if info.type == libvirt.VIR_DOMAIN_JOB_NONE:
                ...
                # We loop every 500ms to don't log on every
                # iteration to avoid spamming logs for long
                # running migrations. Just once every 5 secs
                # is sufficient, as we'll only care about
                # data transfer progress info for long running
                # migration operations
                if (n % 10) == 0:
                    LOG.debug(
                        "Migration running processed=%d remaining=%d total=%d",
                        info.memory_processed, info.memory_remaining,
                        info.memory_total, instance=instance)
                ...
                LOG.debug("Migration operation has completed",
                          instance=instance)
